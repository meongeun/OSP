...
int main() {
    ...
    // Read each image
    I1 = imread("C:\\Users\\user\\Desktop\\KME\\OpenSW\\Lab\\Lab02\\stitchingL.jpg");
    I2 = imread("C:\\Users\\user\\Desktop\\KME\\OpenSW\\Lab\\Lab02\\stitchingR.jpg");
    ...
    for (int i = -diff_x_; i < I_f.rows - diff_x_; i++) {
        for (int j = -diff_y_; j < I_f.cols - diff_y_; j++) {
            ...
            float mu = y - y1;
            float lambda = x - x1;
//범위는 위의 그림에서처럼 1사분면 식 I2의 위치  
            if (x1 >= 0 && x2 < I2_row && y1 >= 0 && y2 < I2_col)
            {
                //Move by diff_x_, diff_y_
                I_f.at<Vec3f>(i + diff_x_, j + diff_y_) = (1 - mu) * ((1 - lambda) * I2.at<Vec3f>(x1, y1) + (lambda)*I2.at<Vec3f>(x2, y1)) + (mu) * ((1 - lambda) * I2.at<Vec3f>(x1, y2) + (lambda)*I2.at<Vec3f>(x2, y2));   
//위의 Bilinear 공식과 동일, diff_x_, diff_y_만큼 움직어야됨 
            }
        }
    }
