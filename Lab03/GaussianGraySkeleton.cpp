			...
			if (!strcmp(opt, "zero-paddle")) {
				for (int a = -n; a <= n; a++) {
					for (int b = -n; b <= n; b++) {
						if ((i + a <= row - 1) && (i + a >= 0) && (j + b <= col - 1) && (j + b >= 0)) {
							...
							output.at<uchar>(i, j) += kernel.at<float>(a + n, b + n) * input.at<uchar>(i + a, j + b);
				...
				for (int a = -n; a <= n; a++) {
					for (int b = -n; b <= n; b++) {
						...
						output.at<uchar>(i, j) += kernel.at<float>(a + n, b + n) * input.at<uchar>(tempa, tempb);
				...
				for (int a = -n; a <= n; a++) {  // Denominator in m(s,t)
					for (int b = -n; b <= n; b++) {
						if ((i + a <= row - 1) && (i + a >= 0) && (j + b <= col - 1) && (j + b >= 0)) {
							sum1 += exp(-(pow(a, 2) / (2 * pow(sigmaS, 2))) - (pow(b, 2) / (2 * pow(sigmaT, 2)))) * (float)(input.at<G>(i + a, j + b));
							sum2 += exp(-(pow(a, 2) / (2 * pow(sigmaS, 2))) - (pow(b, 2) / (2 * pow(sigmaT, 2)))); 
						}
					}
				}
