...
int main() {
	...
	cout << input1.type() << '\t' << input2.type() << endl;
	...
	for (int i = 0; i < keypoints2.size(); i++) {
		...
		kp.pt.x += size1.width;
		circle(matchingImage1, kp.pt, cvRound(kp.size*0.25), Scalar(255, 255, 0), 1, 8, 0);
	}
	...
	for (int i = 0; i < n; i++) {
		ki.push_back(rand() % (int)srcPoints1.size());
	...
	int u = 0;
	vector<int> li;
	cout << "srcPoints1 size: " << srcPoints1.size() << endl;
	Mat A12 = cal_random_affine<float>(srcPoints1, dstPoints1, n, ki);
	
	for (int j = 0; j < srcPoints1.size(); j++) {
		Point2f p12(A12.at<float>(0) * srcPoints1[j].y + A12.at<float>(1) * srcPoints1[j].x + A12.at<float>(2),
			A12.at<float>(3) * srcPoints1[j].y + A12.at<float>(4) * srcPoints1[j].x + A12.at<float>(5));
		cout << (p12.x - dstPoints1[j].x) * (p12.x - dstPoints1[j].x) + (p12.y - dstPoints1[j].y) * (p12.y - dstPoints1[j].y) << '\t' << sigma * sigma << endl;
	...
	//ll.size()
	Mat A12 = cal_random_affine<float>(srcPoints1, dstPoints1, ll.size(), ll);
	Mat A21 = cal_random_affine<float>(dstPoints1, srcPoints1, ll.size(), ll);
...
template <typename T>
Mat cal_affine(vector<Point2f> srcPoints, vector<Point2f> dstPoints, int number_of_points) {
	cout << "cal_affine1" << endl;
	Mat M(2 * number_of_points, 6, CV_32F, Scalar(0));
	Mat b(2 * number_of_points, 1, CV_32F, Scalar(0));
	Mat M_trans, temp, affineM;

	// initialize matrix
	cout << "cal_affine2" << endl;
	cout << srcPoints << endl;
	for (int i = 0; i < number_of_points; i++) {
